...
pinMode(3, INPUT_PULLUP); // Salida 4
pinMode(4, INPUT_PULLUP); // Salida 6
pinMode(5, INPUT_PULLUP); // salida 7
pinMode(6, INPUT_PULLUP); // salida 8
pinMode(7, INPUT_PULLUP); // salida 9
pinMode(8, INPUT_PULLUP); // salida 10
pinMode(9, INPUT_PULLUP); // salida 11
pinMode(10, INPUT_PULLUP); // salida 12
pinMode(11, INPUT_PULLUP); // run config
pinMode(12, INPUT_PULLUP); // Temperatura
pinMode(15, INPUT_PULLUP); // salida 5
pinMode(21, INPUT_PULLUP); // salida 1
pinMode(17, INPUT_PULLUP); // salida 3
pinMode(20, INPUT_PULLUP); // salida 2
pinMode(16, OUTPUT); // rst de modulo
...
//////////////// FUNCION PARA ENVIAR LOS DATOS //////////////////////////////////
   bool enviardata(){
    ...
    if (inet.connectTCP("api.thingspeak.com", 80)==1){ 
     ...
     gsm.SimpleWrite("T6MTJ6PTV8481IK5\r\n"); //APIKEY
...
attachInterrupt(0, cero, FALLING); // activar la interrupcion de la frecuencia
Tiempo=millis();
Contador=0;
while(abs(millis()-Tiempo)<1999){
  delay(1);}
detachInterrupt(0);// desactiva la interrupcion de la frecuencia 
...
bool DetectarEvento(int n){
  ...
  EEPROM.get(85,auxx);
  PtaPrin=digitalRead(21);
  if(auxx==0){PtaPrin=not(digitalRead(21));}  // entrada 1 Pta princi
  EEPROM.get(90,auxx);
  PtaGenDC=digitalRead(20);
  if(auxx==0){PtaGenDC=not(digitalRead(20));} // entrada 2 Pta Gen DC
  EEPROM.get(95,auxx);
  PtaChelf=digitalRead(17);
  if(auxx==0){PtaChelf=not(digitalRead(17));}; // entrada 3 Pta chelter  
  EEPROM.get(100,auxx);
  Mov=digitalRead(3);
  if(auxx==0){Mov=not(digitalRead(3));}      // entrada 4 movimento
  EEPROM.get(105,auxx);
  Vibra=digitalRead(15);
  if(auxx==0){Vibra=not(digitalRead(15));}     // entrada 5 sensor de vibracion
  EEPROM.get(110,auxx);
  GenOn=digitalRead(4);
